...
def query_user_status_date(username, status_key, date):
    dynamodb = boto3.resource('dynamodb')
    ...
    response = table.query(
        ...
        KeyConditionExpression=Key('pk').eq('#USER#{0}'.format(username)) & 
                               Key('status_date').eq('#{0}#{1}'.format(status_key.upper(), date))
    )
    return response['Items']
    

def query_user_status(username, status_key):
    dynamodb = boto3.resource('dynamodb')
    ...
    response = table.query(
        IndexName='user-status-date-index',
        KeyConditionExpression=Key('pk').eq('#USER#{0}'.format(username)) & 
                               Key('status_date').begins_with('#{0}'.format(status_key.upper()))
    )
    return response['Items']


    
def query_user_date(username, date):
    dynamodb = boto3.resource('dynamodb')
    ...
    pending_response = table.query(
        IndexName='user-status-date-index',
        KeyConditionExpression=Key('pk').eq('#USER#{0}'.format(username)) & 
                               Key('status_date').begins_with('#PENDING#{0}'.format(date))
    )
    
    placed_response = table.query(
        IndexName='user-status-date-index',
        KeyConditionExpression=Key('pk').eq('#USER#{0}'.format(username)) & 
                               Key('status_date').begins_with('#PLACED#{0}'.format(date))
    )
    
    response = pending_response['Items']
    response.extend(placed_response['Items'])
